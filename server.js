...
app.post('/blood_donors/all',async (req,res)=>{
    ...
    const bloodDonors=await DonorRegistration.find({ organisation:organisation,donationStatus:"registered"});
    if(bloodDonors){
...
app.post('/expired_blood',async (req,res)=>{
    ...
    if(cMonth<10) cMonth="0"+cMonth;
    let today=cYear+"-"+cMonth+"-"+cDate;
    const expiredBlood=await BloodComponet.find({organisation:req.body.organisation,expiryDate:{$lt:today}});
    let currentDate = new Date(Date.now());
...
app.post('/expired_blood/all',async (req,res)=>{
    ...
    if(expiredBlood){
        res.status(200).send(expiredBlood);
    }
    else{
